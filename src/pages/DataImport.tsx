              ...
              {waImportMode === 'bulk' ? (
                ...
                <div className="space-y-4">
                  ...
                  <div className="p-4 rounded-lg border border-border bg-muted/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-sm font-medium text-foreground">Estado de conexión Meta API</span>
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={checkWaApiStatus}
                        disabled={waApiStatus.checking}
                      >
                        {waApiStatus.checking ? (
                    ...
                    {waApiStatus.checking ? (
                      ...
                      <div className="space-y-2">
                        <div className="flex items-center gap-2 p-2 rounded border border-green-500/30 bg-green-500/10">
                          ...
                          <span className="text-xs font-medium text-green-700 dark:text-green-400">Conectado a WhatsApp Business API</span>
                        </div>
                        <div className="grid grid-cols-2 gap-2 text-xs">
                          {waApiStatus.verified_name && (
                            <div className="p-2 rounded border border-border bg-background">
                              <div className="text-muted-foreground">Nombre verificado</div>
                              <div className="font-medium text-foreground">{waApiStatus.verified_name}</div>
                            </div>
                          )}
                          {waApiStatus.phone_number && (
                            <div className="p-2 rounded border border-border bg-background">
                              <div className="text-muted-foreground">Teléfono</div>
                              <div className="font-medium text-foreground">{waApiStatus.phone_number}</div>
                            </div>
                          )}
                          {waApiStatus.quality_rating && (
                            <div className="p-2 rounded border border-border bg-background">
                              <div className="text-muted-foreground">Calidad</div>
                              <div className="font-medium text-foreground">{waApiStatus.quality_rating}</div>
                            </div>
                          )}
                          <div className="p-2 rounded border border-border bg-background">
                            <div className="text-muted-foreground">Webhook suscrito</div>
                            <div className={cn("font-medium", waApiStatus.webhook_subscribed ? "text-green-600" : "text-destructive")}>
                              {waApiStatus.webhook_subscribed ? '✓ Sí' : '✗ No'}
                            </div>
                          </div>
                        </div>
                        {!waApiStatus.webhook_subscribed && (
                          <div className="p-2 rounded border border-yellow-500/30 bg-yellow-500/10 text-xs text-yellow-700 dark:text-yellow-400">
                            ⚠ El webhook no está suscrito. Los mensajes no llegarán. Configúralo en <a href="https://developers.facebook.com/apps/" target="_blank" rel="noopener noreferrer" className="underline font-medium">Meta Developers</a> → Tu App → WhatsApp → Configuración.
                          </div>
                        )}
                      </div>
                    ) : waApiStatus.connected === false ? (
                      <div className="space-y-3">
                        ...
                        <div className="p-3 rounded border border-border bg-background text-xs space-y-2">
                          <p className="font-medium text-foreground">{waApiStatus.message || 'No se pudo conectar'}</p>
                          {waApiStatus.error === 'token_expired' && (
                            <div className="space-y-1.5 text-muted-foreground">
                              <p className="font-medium text-foreground">Para reconectar:</p>
                              <ol className="list-decimal pl-4 space-y-1">
                                <li>Ve a <a href="https://developers.facebook.com/apps/" target="_blank" rel="noopener noreferrer" className="underline text-primary">Meta Developers</a></li>
                                <li>Selecciona tu app → WhatsApp → Configuración de la API</li>
                                <li>Genera un nuevo <strong>Token de acceso permanente</strong></li>
                                <li>Actualiza el secreto <code className="bg-muted px-1 rounded">WHATSAPP_API_TOKEN</code> en Supabase</li>
                              </ol>
                            </div>
                          )}
                          {waApiStatus.error === 'missing_credentials' && (
                            <div className="space-y-1.5 text-muted-foreground">
                              <p>Faltan credenciales de la API de WhatsApp Business. Configura los secretos en Supabase:</p>
                              <ul className="list-disc pl-4 space-y-0.5">
                                <li><code className="bg-muted px-1 rounded">WHATSAPP_API_TOKEN</code></li>
                                <li><code className="bg-muted px-1 rounded">WHATSAPP_PHONE_ID</code></li>
                              </ul>
                            </div>
                          )}
                        </div>
                    ...
                    {/* Webhook URL for easy copy */}
                    <div className="space-y-1.5">
                      <div className="text-xs text-muted-foreground">URL del webhook (para configurar en Meta):</div>
                      <div className="flex items-center gap-2">
                        <code className="flex-1 text-[10px] p-2 rounded border border-border bg-background text-foreground break-all select-all">
                          https://xfjlwxssxfvhbiytcoar.supabase.co/functions/v1/whatsapp-webhook
                        </code>
                        <Button
                          size="sm"
                          ...
                          className="shrink-0"
                          onClick={() => {
                            navigator.clipboard.writeText('https://xfjlwxssxfvhbiytcoar.supabase.co/functions/v1/whatsapp-webhook');
