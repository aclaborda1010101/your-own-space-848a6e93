...
/**
 * Card — upgraded to the holo design system.
 * - Subtle inner highlight + ambient tint
 * - Glass border + hover lift
 * - Keeps full API compatibility with shadcn/ui
 */
const Card = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "relative rounded-2xl border border-border/60 bg-card/80 text-card-foreground",
        "backdrop-blur-xl shadow-md",
        "before:absolute before:inset-0 before:rounded-2xl before:pointer-events-none",
        "before:bg-[linear-gradient(135deg,hsl(var(--primary)/0.04)_0%,transparent_40%,hsl(var(--accent)/0.03)_100%)]",
