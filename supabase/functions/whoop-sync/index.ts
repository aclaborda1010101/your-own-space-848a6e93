...
async function fetchWhoopDataForDateRange(
  ...
  // Fetch recovery with pagination
  try {
  ...
  // Fetch cycles (strain) with pagination
  try {
    const records = await fetchAllPages(
...
serve(async (req) => {
  ...
  try {
    ...
    const { action, userId, days = 30 } = await req.json();
    ...
    if (action === "backfill" && userId) {
      ...
      // Fetch in batches of 7 days to avoid timeouts
      const allRows: any[] = [];
      for (let i = 0; i < daysToBackfill; i += 7) {
        ...
        console.log(`[whoop-sync] Backfill batch ${batchStart.toISOString().split("T")[0]} to ${batchEnd.toISOString().split("T")[0]}`);
      ...
      if (allRows.length > 0) {
        ...
        for (let i = 0; i < allRows.length; i += 50) {
          ...
          const { error: upsertErr } = await supabase
            .from("whoop_data")
            .upsert(chunk, { onConflict: "user_id,data_date" });
          if (upsertErr) console.error("Backfill upsert error:", upsertErr);
        }
      }

      console.log(`[whoop-sync] Backfill complete: ${allRows.length} days synced out of ${daysToBackfill} requested`);
